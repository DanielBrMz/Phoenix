...
const generateRandomCoords = (center: [number, number]) => {
...
const generateAlertsForReceiver = (numberOfAlerts: number, receivedAtStartingPoint: Date): Alert[] => {
  ...
  for (let i = 0; i < numberOfAlerts; i++) {
    ...
    const sentAt = new Date(receivedAt.getTime() - hoursToSendAlertInAdvance[i % hoursToSendAlertInAdvance.length] * 3600 * 1000);
...
export const generateMockReceivers = (centerCoord: [number, number], n: number): Receiver[] => {
  const receivers = Array.from({ length: n }, (_, i) => {
    const id = String(i);
    const coords = generateRandomCoords(centerCoord);
